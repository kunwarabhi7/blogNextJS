...
<nav className=" px-2 sm:px-4 py-2.5 dark:bg-gray-900 fixed w-full z-20 top-0 left-0 border-b border-gray-200 dark:border-gray-600">
  ...
  <div className="items-center justify-between  w-full md:w-auto md:order-1 " id="navbar-sticky">
    <ul className="flex flex-row  mt-3 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
      <li className=' flex py-2 pl-3 pr-4 text-white  rounded md:bg-transparent  md:p-0 dark:text-white"'>
        <h1 className='mt-2 text-sm  md:text-base' >Welcome, {user.displayName} </h1>
      <img src={user.photoURL} className=" w-9 sm:w-9  mx-4 mr-3 h-9 mt-4 sm:h-9 md:mt-2 rounded-full" alt="Flowbite Logo" />
